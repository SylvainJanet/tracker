...
@SqliteTestDatabase("daily-record-open-api")
class DailyRecordOpenApiTest {
    ...
    void openApiMatchesContract() throws Exception {
        mockMvc.perform(get("/v3/api-docs"))
                ...
                .andExpect(jsonPath("$.paths['/api/daily-records'].post.responses['201']").exists())
                .andExpect(jsonPath("$.paths['/api/daily-records'].post.responses['400']").exists())
                .andExpect(jsonPath("$.paths['/api/daily-records'].post.responses['409']").exists())
                .andExpect(
                        jsonPath("$.paths['/api/daily-records'].post.responses.length()").value(3))
                .andExpect(
